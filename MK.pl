...
#		-does not use sites that include codons with ambiguity
...
## could make a key for how many outgroup lines - could be many more than two
## do I want to count polymorphisms for two different lines?
## currently this script counts polymorphisms for a single species

my %mut_code;
&initialize;

my ($opt_outfile, $opt_ingroup, @opt_outgroup, $opt_pol, $opt_dir);

GetOptions('outfile:s' => \$opt_outfile,
			'outgroup:s' => \@opt_outgroup,
			'ingroup:s' => \$opt_ingroup,
			'pol:s' => \$opt_pol,
			'dir:s' => \$opt_dir,);

if($opt_pol){
	$opt_pol = substr($opt_pol, 0, 3);
}
else{
	usage();
}

unless ($opt_outfile && scalar @opt_outgroup && $opt_ingroup){
	usage();
}

my $num_outs = scalar @opt_outgroup;

if($opt_pol ne 'pol' && $num_outs > 1){	## too many outs
	print "You've selected an unpolarized analysis, but have specified more than one outgroup.\n";
	print "I can't figure out what to do, so I quit!\n";
	die;
}
if($opt_pol eq 'pol' && $num_outs != 2){ ## not the right number of outs
	print "You've selected a polarized analysis, but have specified $num_outs outgroup(s).\n";
	print "You need to specify 2 outgroups.\n";
	print "I can't figure out what to do, so I quit!\n";
	die;
}

open (OUT, ">$opt_outfile") || die "Please specify an outfile\n";

##OPEN DIRECTORY WITH ALIGNMENT FILES
opendir(DIR, $opt_dir);
my @allfiles = readdir DIR;
my $numfiles = scalar @allfiles;

#print OUT "file\tNSpoly\tNSfix\tSpoly\tSfix\tMKcodons\tavgsample\tFETpval\n";
...
while ($a<$numfiles){
	...
	if($allfiles[$a] =~ /.fa$/){
		my $datafile = $opt_dir.$allfiles[$a];
		open (FASTAINPUT, $datafile) || die "Can't open $datafile: $!\n";
		
		$/ = '>';
		($ID, $SEQ, $outSEQ) = parseINOUT();
		$/ = "\n";
		if($outSEQ->[0] && scalar @{$outSEQ} == $num_outs && $SEQ->[1]){
			calcMK($SEQ, $outSEQ);
		}
		close FASTAINPUT;
	}
...
sub parseINOUT{
	...
	<FASTAINPUT>;
	while(my $data = <FASTAINPUT>){
		
		(my $tempSP, my $tempID) = ($data =~ /^\s?(\S+);\s?(\S+)/);
		
		if($tempSP eq $opt_ingroup){
...
sub calcMK{
			...
			if($ins > 1 && scalar @codons){
				if($difffromOUT){
					if($INsame == $ins-1){	## fixed diff from mel
						...
						my ($tNdiff, $tSdiff) = codon2codon($pair);
						$Nfix += $tNdiff;
						$Sfix += $tSdiff;
						#print "$tNdiff\t$tSdiff\n";
					}
						...
						if($sameasOUT){	## 2 sim codons, 1 sameasOUT, 1 diff, calc path for diff add to poly
							...
							if(scalar @codons == 3){  # number changed from 2 /JV
							...
							my ($tNp, $tSp) = codon2codon($polypair);
							$Npoly += $tNp;
							$Spoly += $tSp;
						}
					}
	...
	if($Npoly || $Spoly || $Nfix || $Sfix){		
		...
		print OUT $allfiles[$a], "\t", $Npoly, "\t", $Nfix, "\t", $Spoly, "\t", $Sfix, "\t", 
				$numcodons, "\t", $FET_pvalue, "\n";
...
#   Version 3, using hash lookup, from Chapter 8 in Perl Bioinf. Book
...
MK.pl 	-outfile [outfile.txt]
	-pol [polarized|unpolarized]
	-outgroup [speciesID] # 1 for unpolarized; must have 2 for polarized analysis
	-ingroup [speciesID] # 1 species
	-dir [directory where fasta files are located] # make sure to use "./" at beginning
			
example FASTA file format - must have species id as first field followed by semicolon
	ok to have interleaved or not, file names must be *.fa
...
e.g. for polarized analysis

	perl MK.pl -outfile out.txt -pol pol -outgroup mel -outgroup yak -ingroup sim -dir ./your_fasta_directory

e.g. for unpolarized anlaysis

	perl MK.pl -outfile out.txt -pol unpol -outgroup mel -ingroup sim -dir ./your_fasta_directory
